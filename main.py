...
# get input value for input.image(value11)
# returns base64-encoded image data
image = io.get('input.image(value11).current')

# get input value for input.integer(prunings)
prunings = int(io.get('input.integer(prunings).current'))

# get input value for input.integer(block_size_width)
block_size_width = int(io.get('input.integer(block_size_width).current'))

# get input value for input.integer(block_size_length)
block_size_length = int(io.get('input.integer(block_size_length).current'))

# get input value for input.choice(threshold)
threshold = io.get('input.choice(threshold).current')

# get input value for input.integer(cutoff)
cutoff = int(io.get('input.integer(cutoff).current'))

# get input value for input.integer(order)
order = int(io.get('input.integer(order).current'))

# get input value for input.integer(Ntheta)
Ntheta = int(io.get('input.integer(Ntheta).current'))

# get input value for input.integer(nbins)
nbins = int(io.get('input.integer(nbins).current'))
...
def load_image(img, display=False):
	...
	# Reading an image taken from Rappture
    imgEnc = base64.b64encode(img)
    imgDec = base64.b64decode(imgEnc)
    npimg = fromstring(imgDec, dtype=uint8)
    raw_image = cv2.imdecode(npimg, 1)
...
################################################################################
# Rappture GUI Functions

def plot_to_image(image):
	"""	
	Given an array, draw it on a matplotlib plot and convert it to a b64 encoded
	image
	"""
	...
	pyplot.savefig(my_stringIObytes)
	my_stringIObytes.seek(0)
	b64_image = base64.b64encode(my_stringIObytes.read())
	return(b64_image)

def graph_to_image(orientation, nbins, theta, p, p0):
	"""
	Given data on the nanowires, plot a p(theta) vs. theta graph and convert it to
	a b64 encoded image
	"""
...
block_size = (block_size_width, block_size_length)             
                
grey_image = load_image(image, display=False)
grey_image_return = base64.encodestring( cv2.imencode('.jpg', grey_image)[1].tostring() )

bw_image = digitize_image(grey_image, block_size, threshold=threshold, display=False)
bw_image_return = plot_to_image(bw_image)

skel_image = skeletonize_image(bw_image, display=False)
skel_image_return = plot_to_image(skel_image)
...
unweighted_graph_return = graph_to_image(orientation, nbins, theta, p, p0)
weighted_graph_return = graph_to_image(orientation_weighted, nbins, theta, pw, pw0)

sUnweighted = str(S[0]) + "\n" + "\n" + array2string(S)
sWeighted = str(Sw[0]) + "\n" + "\n" + array2string(Sw)

# Convert to CSV
nanowireDataArray = ( column_stack( ( length, orientation, orientation_weighted ) ) )

nanowireData = "Nanowire Length (in pixels), S (Unweighted), S (Weighted) \n"
counter = 0

# Format a string based off a 3 column table
for i in nditer(nanowireDataArray):
	if counter != 2:
		nanowireData += str(i) + ","
		counter += 1
	else:
...
# save output value for output.image(greyImage)
# data should be base64-encoded image data
io.put('output.image(greyImage).current', grey_image_return)

# save output value for output.image(bwImage)
# data should be base64-encoded image data
io.put('output.image(bwImage).current', bw_image_return)

# save output value for output.image(skelImage)
# data should be base64-encoded image data
io.put('output.image(skelImage).current', skel_image_return)

# save output value for output.image(graphUnweighted)
# data should be base64-encoded image data
io.put('output.image(graphUnweighted).current', unweighted_graph_return)

# save output value for output.string(nanowireData)
io.put('output.string(nanowireData).current',nanowireData)

# save output value for output.image(graphWeighted)
# data should be base64-encoded image data
io.put('output.image(graphWeighted).current', weighted_graph_return)

# save output value for output.string(sUnweighted)
io.put('output.string(sUnweighted).current', sUnweighted)

# save output value for output.string(sWeighted)
io.put('output.string(sWeighted).current',sWeighted)
